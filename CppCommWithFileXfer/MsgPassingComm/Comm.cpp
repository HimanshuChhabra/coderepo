...
const std::string sendFilePath = "../Storage";
const std::string clientRecevingPath = "../../../client_stage";
...
void Sender::start()
{
  std::function <void()> threadProc = [&]() {
    while (true)
    {
      ...
      if (msg.to().address != lastEP.address || msg.to().port != lastEP.port || msg.command() == "connectionReq")
      {
        connecter.shutDown();
        //connecter.close();
...
bool Sender::sendFile(Message msg)
{
  if (!msg.containsKey("file"))
    return false;
  std::string fileSpec  = "";
  if (msg.value("sender") == "server") {
	  fileSpec = sendFilePath + "/" + msg.file();				// Transfer from root to storage and then to client (no need for nmsp)
  }
  else if (msg.value("sender") == "client") {
	 fileSpec = msg.file();				// client always gives full qualified path
  }
...
public:
  ...
  */
  bool receiveFile(Message msg)
  {
	...
	std::string fileSpec;
	// server to client
	if (msg.value("sender") == "server") {		// receiver is client
		fileSpec = clientRecevingPath + "/" + msg.value("filenames");
	}
	else if (msg.value("sender") == "client") {		// receiver is Storage
		//fileSpec = sendFilePath + "/" + msg.value("namespaces") + "/"+ msg.value("filename");
		fileSpec = sendFilePath + "/" + msg.value("filename");
	}
