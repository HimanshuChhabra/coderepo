...
namespace Repository
{
  ...
  const SearchPath storageRoot = "../root";  // root for all server file storage
  ...
  public:
	...
	void checkFileSendStatus(MsgPassingCommunication::Message msg);
  ...
  inline void Server::checkFileSendStatus(MsgPassingCommunication::Message msg) {
	  if (msg.containsKey("file")) {
		  msg.remove("file");
		  msg.attribute("content", "File Received");
		  msg.command("file_read");
		  postMessage(msg);
	  }
  }
