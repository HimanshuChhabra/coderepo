...
bool testR3b(DbCore<PayLoad>& db)
{
  ...
  demoElem.name("Ammar");demoElem.descrip("TA for CSE687");demoElem.payLoad(PayLoad("You should try ..."));db["Salman"] = demoElem;
  if (!db.contains("Salman"))
    return false;
  demoElem.name("Jianan"); demoElem.payLoad(PayLoad("Dr. Fawcett said ..."));
  db["Sun"] = demoElem;
  demoElem.payLoad(PayLoad("You didn't demonstrate Requirement #4")); demoElem.name("Nikhil");
  db["Prashar"] = demoElem;
  demoElem.payLoad(PayLoad("You didn't demonstrate Requirement #5"));demoElem.name("Pranjul");
  db["Arora"] = demoElem;
  if (db.size() != 5)
    return false;
  std::cout << "\n  after adding elements with keys: ";
  Keys keys = db.keys(); showKeys(db); Utilities::putline();
  std::cout << "\n  make all the new elements children of element with key \"Fawcett\"";
  db["Fawcett"].children().push_back("Salman");db["Fawcett"].children().push_back("Sun");
  db["Fawcett"].children().push_back("Prashar");db["Fawcett"].children().push_back("Arora");
  showHeader(); showElem(db["Fawcett"]);
  db["Salman"].children().push_back("Sun"); db["Salman"].children().push_back("Prashar");
  db["Salman"].children().push_back("Arora");
  Utilities::putline();  std::cout << "\n  showing all the database elements:";
  showDb(db); Utilities::putline();
  std::cout << "\n  database keys are: "; showKeys(db);
  Utilities::putline();
  return true;
}
//----< demo Requirement #6 >------------------------------------------
